...
async function htaDeterministic(body: any) {
  const { population, intervention, comparison, outcome, country, countryLabel, icer, costEffective, currency, evidenceCertainty } = body
...
export async function POST(req: NextRequest) {
  try {
    ...
    const SYSTEM_PROMPT = `You are a Health Technology Assessment (HTA) expert using the EUnetHTA 9-domain model. Analyze the given intervention and generate a comprehensive HTA response with:
- 9 domain assessments (description, health problem, clinical effectiveness, safety, cost-economic, organizational, ethical, social, legal)
- Each domain with judgment (favorable/uncertain/unfavorable), evidence text, progress score (0-100), and criteria met/not met
- Overall recommendation with conditions and monitoring requirements
- Budget impact analysis for years 1, 3, 5
- Clear summary text

Return ONLY valid JSON matching the exact structure (no markdown, no escaping):
{ "technologyName": string, "maturityStage": string, "domains": {...}, "overallJudgment": {...}, "budgetImpact": {...}, "summaryText": string }`

    const userPrompt = `Population: ${population}
Intervention: ${intervention}
Comparison: ${comparison || 'Standard of care'}
Outcome: ${outcome || 'Primary outcome'}
Country: ${country || 'SA'} (${countryLabel || 'Saudi Arabia'})
Evidence certainty: ${evidenceCertainty || 'moderate'}
ICER: ${icer ? `${icer.toLocaleString()} ${currency}/QALY` : 'Not provided'}
Cost-effective: ${costEffective}`
    ...
    const result = await htaDeterministic(body)
